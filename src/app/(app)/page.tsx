...
interface DashboardData {
  ...
  porProjeto: Array<{
    ...
    saldoReceberBemVendido: number; saldoCaixa: number; saldoAPagar: number; projetadoFinal: number
  }>
}
              ...
              <table className="w-full text-sm min-w-[840px]">
                ...
                <tbody className="divide-y divide-slate-50">
                  {data.porProjeto.map(p => {
                    const isOp = p.status === "operacional"
                    const st = PROJECT_STATUS[p.status]
                    return (
                      <tr key={p.id} className={isOp ? "bg-slate-50/70" : undefined}>
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-2 min-w-0">
                            <span className={`text-xs font-medium text-slate-700 truncate ${isOp ? "italic" : ""}`}>{p.name}</span>
                            {isOp
                              ? <Badge className="bg-slate-200 text-slate-600 text-[9px] px-1.5 py-0 shrink-0">Sem projeto</Badge>
                        ...
                        <td className="px-3 py-2.5 text-right">
                          <p className="text-xs font-bold text-amber-600">{formatCurrency(p.saldoAReceber)}</p>
                          {p.saldoReceberBemVendido > 0 && (
                            <p className="text-[10px] text-slate-400">(+{formatCurrency(p.saldoReceberBemVendido)} venda de bem)</p>
                          )}
                        </td>
